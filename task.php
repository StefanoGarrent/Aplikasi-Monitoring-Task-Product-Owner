...
<body class="bg-[#F0F2F5] flex">
    ...
    <main class="flex-1 p-10">
        <div class="flex justify-between items-end mb-8">
            <div><h2 class="text-2xl font-bold text-gray-800">Monitoring Task</h2></div>
            ...
            <table class="w-full border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php if (mysqli_num_rows($resTask) > 0): ?>
                        ...
                        <tr class="<?= $row['task_url'] != '-' ? 'bg-green-100/50 hover:bg-green-200/50' : 'hover:bg-gray-50' ?> transition duration-150">
                            ...
                            <td class="px-4 py-4 text-center">
                                <?php if ($row['task_url'] != '-'): ?>
                                    <input type="checkbox" class="w-4 h-4 accent-[#00D285] cursor-pointer" <?= $row['status'] == 'Sudah dicek' ? 'checked' : '' ?> onchange="toggleStatus(<?= $row['id'] ?>, this.checked)">
                                <?php else: ?>
                                    <i class="fas fa-lock text-gray-300 text-[10px]" title="Link belum tersedia"></i>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-4 text-center">
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr><td colspan="9" class="p-10 text-center text-gray-400 italic text-sm">Data tidak ditemukan</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    function toggleStatus(id, checked) {
        let status = checked ? 'Sudah dicek' : 'Belum di cek';
        fetch(`services/update_status.php?id=${id}&status=${status}`)
        .then(res => { if(!res.ok) alert('Gagal update status!'); });
    }
    </script>
</body>
</html>